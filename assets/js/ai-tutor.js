...
(function () {
  ...
  function renderQuota(q) {
    ...
    elQuota.innerHTML =
      ...
      "完整模考 <b>" + q.fullMockLeft + "</b>/" + q.fullMockLimit;
  }
  ...
  function emptyState() {
    elMsgs.innerHTML =
      ...
      (mode === "examiner"
        ? "点「新建」开始一场口语模拟。考官全程英文；结束后会给出四项分数。"
        : "点「新建」开始与老师对话。可问口语思路，或直接粘贴写作 Task 1/2 作文请老师批改。") +
      "</div>";
  }
  ...
  function loadQuota() {
    return api("/api/ai-tutor/quota").then(function (d) {
      renderQuota(d.quota);
    }).catch(function () {
      elQuota.textContent = "额度加载失败";
    });
  ...
  function syncModeUI() {
    ...
    document.querySelectorAll(".ai-tutor__exam .chip").forEach(function (b) {
      b.classList.toggle("is-active", b.getAttribute("data-exam") === examType);
    });
  }
  ...
  function createSession() {
    ...
    busy = true;
    hint("正在创建会话…");
    return api("/api/ai-tutor/sessions", {
      method: "POST",
      body: { mode: mode, examType: mode === "examiner" ? examType : "" }
    }).then(function (d) {
      ...
      hint(mode === "examiner" ? "考官已开始，用文字或麦克风作答。" : "直接提问或粘贴作文即可。");
      return loadSessions();
    }).catch(function (e) {
  ...
  syncModeUI();
  emptyState();
  Promise.all([loadQuota(), loadSessions()]).then(function (res) {
    var sessions = null;
    // loadSessions doesn't return list to us — re-fetch lightly via DOM or recreate
    return api("/api/ai-tutor/sessions").then(function (d) {
      sessions = d.sessions || [];
      ...
      // ponytail: auto-start first visit so user isn't stuck on empty state

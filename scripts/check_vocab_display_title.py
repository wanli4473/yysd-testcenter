#!/usr/bin/env python3
"""Assert vocab display titles resolve to 高中/四级/雅思词汇单元N for all manifest items."""
...
def main() -> int:
    ...
    for it in vocab:
        ...
        if not re.fullmatch(r"(高中|四级|雅思)词汇单元\d+", got):
    ...
    if bad:
        ...
        for row in bad[:12]:
            print(" ", row)
        return 1
    print(f"ok {len(vocab)} vocab titles → 高中/四级/雅思词汇单元N")
    return 0
